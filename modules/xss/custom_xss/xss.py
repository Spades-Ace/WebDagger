...
class XSS_Scanner:
    ...
    def xss_submit(
        ...
        if parsed[0]["method"] == "get":
            try:
                ...
                if payload in c:
                    return (True, {"reflection":XSS_Scanner.find_xss_context(c, payload),"p_o_c":p_o_c},any(s in c for s in Common_Variables.sql_errors),any(s in c for s in Common_Variables.xml_parser_errors),any(s in c for s in Common_Variables.fetch_url_errors),any(s in c for s in Common_Variables.open_file_errors),p_o_c)
    ...
    def xss_forms(
        ...
        def send_to_flask(data):
            ...
            requests.post('http://localhost:5000/listen', data=json_data, headers=headers)
        ...
        if hu == True:
            ...
            for l1 in fom:
                ...
                if pre_apyload == True:
                    xp = xp_f.format(
                        XSS_Scanner.hexadecimal_encoder(js_function, random_level=unicode_random_level)
                    )
                else:
                    xp = xp_f
                lst = {}
                    ...
                    for x in l1["inputs"]:
                        if (
                            ...
                            if (
                                ...
                                xss_res = XSS_Scanner.xss_submit(
                                    parsed_form,
                                    xp,
                                    replaceble_parameters,
                                    debug=debug,
    ...
    def scan(
        ...
        for pl in payloads:
            for x in pages:
                if logs==True:
                    print('\n\nPage: {}\n'.format(x))
                    print("Inside custom XSS scanner")
                l.append(XSS_Scanner.xss_forms(x,
                                payload=pl,
        ...
        for x in l:
            if x !=None:
                ...
                if n['result']!=[]:
                    f.append(n)
        return {'result':f,'start_date':started_at,'end_date':time.time()}
